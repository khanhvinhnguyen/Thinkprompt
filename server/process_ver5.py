...
def create_uppercase_pdf(text_details, images, output_file):
    ...
    for img_path, bbox, page_num in images:
        ...
        x0, y0, x1, y1 = bbox
        y = height - y1
        c.drawImage(img, x0, y, width=(x1 - x0), height=(y1 - y0))
...
if input_file.lower().endswith('.pdf'):
    ...
    full_text, images = extract_pdf_text_images(input_file, output_dir)
    uppercase_text = convert_text_to_uppercase(full_text)
    create_uppercase_pdf(uppercase_text, images, output_file)
elif input_file.lower().endswith('.docx'):
    output_dir = os.path.dirname(output_file)
    ensure_dir_exists(output_dir)

    full_text, images = extract_docx_text_images(input_file, output_dir)
    uppercase_text = convert_text_to_uppercase(full_text)
    # TODO: Create DOCX with uppercase text and original formatting
